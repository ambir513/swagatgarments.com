"use client";
import { Moon, MoonStar, Sun, SunIcon } from "lucide-react";
...
type StartPosition =
  ...
  | "bottom-left"
  | "bottom-right";
export interface ThemeAnimateProps {
  theme?: "light" | "dark";
  showLabel?: boolean;
...
export const ThemeAnimate = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        " rounded-full border-1 dark:border-neutral-800 p-1.5 hover:bg-muted-foreground/15 relative overflow-hidden transition-all",
        showLabel && "gap-2",
        className
      )}
    ...
    >
      <audio ref={audioRef} className="hidden" preload="auto">
        <source src="/audio/click-sound.wav" type="audio/wav" />
      </audio>
      {theme === "light" ? (
